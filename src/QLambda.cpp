...
void QLambda::train(const VectorXd & features, const int & action, const double & reward, const VectorXd & newFeatures)
{
	...
	//double bestActionValue=getAction(features,generator);
	//newQ=newFeatures.dot(w.segment(numFeatures*bestActionValue, numFeatures));
    VectorXd qVal(numActions);
    for (int i=0;i<numActions;i++){
        qVal[i]=newFeatures.dot(w.segment(numFeatures*i,numFeatures));
    }
    ...
    delta=reward+gamma*newQ-curQ;
	// Update the e-traces (e).
	// @TODO	@TODO	@TODO	@TODO	@TODO	@TODO	@TODO @TODO	@TODO
	// @TODO 4 OF 9
	// @TODO: Fill in the code to update the vector, e
	for (int i=0;i<numActions*numFeatures;i++){
        e[i]=gamma*lambda*e[i];
	}
    e.segment(numFeatures*action,numFeatures)+=features;
    //e.segment(numFeatures*action,numFeatures)=gamma*lambda*e.segment(numFeatures*action,numFeatures)+features;
