...
ActorCritic::ActorCritic(int numFeatures, int numActions, double alpha_actor, double alpha_critic, double gamma, double lambda)
{
    ...
    w=VectorXd::Zero(numFeatures*numActions);
...
void ActorCritic::train(const VectorXd & features, const int & action, const double & reward, const VectorXd & newFeatures)
{
	...
	VectorXd dlnpi_res(numFeatures*numActions);
	dlnpi_res=dlnpi(features,action);
	eTheta=gamma*lambda*eTheta+dlnpi_res;
	w=w-(alpha_critic*dlnpi_res*(dlnpi_res.dot(w)))+(alpha_critic*delta*eTheta);
	theta=theta+alpha_actor*w;
}
...
VectorXd ActorCritic::dlnpi(const Eigen::VectorXd & features, const int & action)
{
	...
	for (int a=0;a<numActions;a++){
        if (a==action)
            result.segment(numFeatures*action,numFeatures) = (1-actionProbabilities[action])*features;
        else
